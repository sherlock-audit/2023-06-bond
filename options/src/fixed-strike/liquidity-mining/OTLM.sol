...
contract ManualStrikeOTLM is OTLM {
    ...
    function _initialize(bytes calldata params_) internal override {
        ...
        if (strikePrice_ == 0) revert OTLM_InvalidParams();
...
/// @title Oracle Strike Option Token Liquidity Mining (OTLM)
/// @dev The Oracle Strike OTLM contract uses an oracle to determine the fixed strike price when a new option token is created on epoch transition.
...
/// @author Bond Protocol
contract OracleStrikeOTLM is OTLM {
    /* ========== STATE VARIABLES ========== */
    ...
    function _initialize(bytes calldata params_) internal override {
        ...
        if (oracleDiscount_ >= ONE_HUNDRED_PERCENT) revert OTLM_InvalidParams();
        ...
        if (oracle_.currentPrice(quoteToken, payoutToken) == 0) revert OTLM_InvalidParams();
    ...
    /// @inheritdoc OTLM
    function nextStrikePrice() public view override returns (uint256) {
        uint256 discountedPrice = (oracle.currentPrice(quoteToken, payoutToken) *
            (ONE_HUNDRED_PERCENT - oracleDiscount)) / ONE_HUNDRED_PERCENT;
    ...
    function setQuoteToken(ERC20 quoteToken_) external override onlyOwner requireInitialized {
        ...
        if (oracle.currentPrice(quoteToken_, payoutToken) == 0) revert OTLM_InvalidParams();
